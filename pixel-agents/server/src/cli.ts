 ...
 * - Added --provider support with PIXEL_AGENTS_PROVIDER fallback.
 * - Wired the selected provider through AgentRuntime, hook toggles, scanning,
 *   and WebSocket providerCapabilities.
 * - Kept Claude as the default provider while enabling local-agent for
 *   OpenRouter-backed Python workflows.
 */
...
import { copyHookScript, getProviderById } from './providers/index.js';
...
function parseArgs(argv: string[]): CliArgs {
  const args: CliArgs = {
    port: 3100,
    host: '127.0.0.1',
    providerId: process.env['PIXEL_AGENTS_PROVIDER'] ?? 'claude',
  };
...
Options:
  ...
  --provider <id>        Provider to use: claude, local-agent (default: claude)
  ...
  try {
    ...
    const onSetHooksEnabled = async (enabled: boolean): Promise<void> => {
      ...
      if (enabled) {
        ...
        if (provider.id === 'claude') copyHookScript(distRoot);
        console.log(`[Pixel Agents] Hooks installed for provider ${provider.id} (user toggle)`);
      } else {
        await provider.uninstallHooks();
        console.log(`[Pixel Agents] Hooks uninstalled for provider ${provider.id} (user toggle)`);
      }
    };
    ...
    if (runtime.hooksEnabled.current) {
      try {
        ...
        if (provider.id === 'claude') copyHookScript(distRoot);
        console.log(`[Pixel Agents] Hooks installed for provider ${provider.id}`);
      } catch (err) {
    ...
    console.log(
      `\n  Pixel Agents server running at http://${args.host}:${config.port} (provider: ${provider.id})\n`,
    );
