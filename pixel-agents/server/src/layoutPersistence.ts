/**
 * Change log (updated 2026-06-26 17:18 KST):
 * - Added saved-layout validation before accepting ~/.pixel-agents/layout.json.
 * - Rejects malformed layouts where tiles length does not match cols * rows,
 *   preventing the canvas from rendering a giant magenta error floor.
 * - loadLayout now resets invalid saved layouts to the bundled default when one
 *   is available.
 */
...
function validateLayout(layout: Record<string, unknown>): boolean {
  ...
  if (!Array.isArray(tiles) || tiles.length !== cols * rows) return false;
...
export function loadLayout(
  ...
  if (fromFile) {
    if (!validateLayout(fromFile)) {
      console.warn('[Pixel Agents] Saved layout is invalid; resetting to bundled default');
      if (defaultLayout) {
  ...
  function checkForChange(): void {
    ...
    try {
      ...
      if (!validateLayout(layout)) {
        console.warn('[Pixel Agents] Ignoring invalid external layout change');
        return;
      }
