 ...
 * - Allowed hooks-only providers without transcript files to adopt confirmed
 *   external sessions even when no project directory scanner is active.
 *
  ...
  constructor(
    ...
    this.hookEventHandler.setLifecycleCallbacks({
      onExternalSessionDetected: (sessionId, transcriptPath, cwd) => {
        ...
        if (transcriptPath && !isTrackedProjectDir(projectDir) && !this.watchAllSessions.current) {
          return;
        }
        adoptExternalSessionFromHook(
          sessionId,
