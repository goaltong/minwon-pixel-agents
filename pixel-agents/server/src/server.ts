 ...
 * - Added active provider forwarding into the HTTP/WebSocket layer so
 *   providerCapabilities can be emitted for non-Claude providers.
 * - Persisted providerId in server.json and avoided reusing a running server
 *   when it was started for a different provider.
 */
...
export interface ServerConfig {
  ...
  token: string;
  /** Provider id used by this server runtime */
  providerId?: string;
...
export class PixelAgentsServer {
  ...
  async start(options?: {
    ...
    const requestedProviderId = options?.provider?.id;
    const existingProviderId = existing?.providerId ?? 'claude';
    if (
      existing &&
      isProcessRunning(existing.pid) &&
      (!requestedProviderId || existingProviderId === requestedProviderId)
    ) {
    ...
    if (existing && isProcessRunning(existing.pid)) {
      console.log(
        `[Pixel Agents] Existing server PID ${existing.pid} uses provider ${existingProviderId}; starting a separate ${requestedProviderId ?? 'claude'} server.`,
      );
    }
