...
Pixel art office where AI agents (Codex terminals today, any tool tomorrow) become animated characters. Ships as a **VS Code extension** and an **`npx pixel-agents` standalone CLI** from the same source tree.
...
server/                              Lifecycle runtime + Fastify HTTP/WS server
  src/
    providers/hook/Codex/           Reference HookProvider — only place that knows Codex specifics
      Codex.ts                      normalizeHookEvent for 11 Codex events, formatToolStatus, file fallback
      claudeTeamProvider.ts          TeamProvider: reads ~/.Codex/teams/<name>/config.json
      claudeHookInstaller.ts         Atomic install/uninstall in ~/.Codex/settings.json
      constants.ts                   Codex hook event names, script path
...
adapters/vscode/                     VS Code surface — composes core + server
  ...
  agentManager.ts                    Terminal lifecycle (Codex --session-id <uuid>), restore, persist
...
e2e/                                 Playwright suite (real VS Code + mock-Codex scenarios)
  ...
  fixtures/
    ...
    mock-Codex, mock-Codex.cmd     Bash + cmd wrapper invoked instead of real Codex
    mock-Codex-runner.cjs           Scenario runner: appendJsonl, emitHook, holdOpen
  helpers/
    launch.ts                        Electron app + isolated HOME/workspace
    mock-Codex.ts                   claudeScenario() builder
  ...
  tests/
    Codex/hooks-on/                 basic.spec.ts, lifecycle.spec.ts, teams.spec.ts
    Codex/hooks-off/                lifecycle.spec.ts, matrix.spec.ts
...
`HookProvider` (`core/src/provider.ts`) is the integration boundary. Today only Codex is implemented. The interface:
...
Optional extension for CLIs that support team workflows (Codex Agent Teams today). Semantic queries (`discoverTeammates`, `getTeamMembers`, `getTeamMetadataForSession`, `extractTeammateNameFromEvent`, `isTeammateSpawnCall`) — providers choose their own storage strategy.
...
~/.pixel-agents/
  ...
  hooks/Codex-hook.js     Bundled hook script (CJS, shebang)
```
...
JSONL transcripts at `~/.Codex/projects/<project-hash>/<session-id>.jsonl`. Project hash = workspace path with `:`/`\`/`/` → `-`.
...
| ------------------------ | ------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hooks** (preferred)    | Codex Hooks API → HTTP POST → `HookEventHandler` | Instant, reliable. 11 events: `SessionStart`, `SessionEnd`, `Stop`, `PermissionRequest`, `Notification`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `SubagentStart`, `SubagentStop` |
...
| `Codex.test.ts`               | `normalizeHookEvent` per Codex event, file fallback                |
| `claudeHookInstaller.test.ts`  | Atomic install/uninstall                                            |
| `Codex-hook.test.ts`          | Spawned hook script integration (needs `dist/hooks/Codex-hook.js`) |
...
| -------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `spawn`              | `Codex/hooks-on/basic.spec.ts`                                           | "+ Agent" creates JSONL session, character renders, external session adoption                                                                                      |
| `lifecycle`          | `Codex/hooks-on/lifecycle.spec.ts`, `Codex/hooks-off/lifecycle.spec.ts` | /clear, --resume, reassignment, stale cleanup, subagent visibility, background routing                                                                             |
...
| `teams`              | `Codex/hooks-on/teams.spec.ts`                                           | Internal/external lead + tmux/inline teammate routing                                                                                                              |
| `matrix`             | `Codex/hooks-off/matrix.spec.ts`                                         | Heuristic-mode broad coverage                                                                                                                                      |
| `standalone`         | `standalone/hooks.spec.ts`                                                | npx pixel-agents serves SPA, WebSocket protocol, hook-driven lifecycle in browser                                                                                  |

**Mock Codex**: Tests never invoke real `Codex`. A bash script (`e2e/fixtures/mock-Codex`) is copied into an isolated `bin/` and prepended to `PATH`. The scenario runner (`mock-Codex-runner.cjs`) honors `claudeScenario(...).at(ms).appendJsonl(record).emitHook(event).holdOpenFor(ms).build()` to drive timed JSONL writes and hook events.
...
3. **Hook scripts** (`dist/hooks/Codex-hook.js`) from `server/src/providers/hook/Codex/hooks/Codex-hook.ts`. CJS, shebang.
...
- **External-session adoption**: scanner runs every 3 s. In hooks-OFF mode external scenarios, the test setup can race the first scanner tick. Mock-Codex scenarios should give a few seconds of margin before assertions.
...
4. `3-vision-inspect.ts` — Codex vision auto-metadata
...
- **TeamProvider** as optional extension. Codex Agent Teams is the only implementation.
