 ...
 * - Added active provider selection through PIXEL_AGENTS_PROVIDER while keeping
 *   Claude as the default VS Code launch provider.
 * - Routed runtime, hook install toggles, server options, and provider capability
 *   messages through the selected provider.
 * - Limited hook-script copying to the Claude provider because local-agent
 *   receives direct HTTP events from external Python scripts.
 */
...
import { copyHookScript, getProviderFromEnv } from '../../server/src/providers/index.js';
...
export class PixelAgentsViewProvider implements vscode.WebviewViewProvider {
  ...
  private readonly provider = getProviderFromEnv();
  ...
  private initServer(): void {
    ...
    this.pixelAgentsServer
      .start({ store: this.store, provider: this.provider, embedded: true })
      .then((config) => {
        ...
        if (hooksEnabled) {
          ...
          if (this.provider.id === 'claude') copyHookScript(this.context.extensionPath);
        }
        console.log(
          `[Pixel Agents] Server: ready on port ${config.port} (provider: ${this.provider.id})`,
        );
      })
      .catch((e) => {
